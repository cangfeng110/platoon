...
class Factory {
 public:
   ...
   */
  // using  T&&  universal reference to realize perfect transfer;
  ...
  std::unique_ptr<AbstractProduct> CreateObject(const IdentifierType &id,
    ...
    if (id_iter != producers_.end()) {
      return std::unique_ptr<AbstractProduct>(
          (id_iter->second)(std::forward<Args>(args)...));//wan mei zhuan fa
    }
    return nullptr;
  }
   ...
   * @param id The identifier of the class we which to instantiate
   * @param args the object construction arguments
   * @note this method is despited!
   */
  template <typename... Args>
  std::unique_ptr<AbstractProduct> CreateObject_Old(const IdentifierType &id,
                                                Args... args) {
    auto result = CreateObject(id, args...);
    //IF(!result) << "Factory could not create Object of type : " << id;
    return result;
  }
