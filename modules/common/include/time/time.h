...
/**
 * @namespace atd::common::time
 * @brief atd::common::time
 */
namespace atd
{
namespace common
 ...
 * @return a doule in seconds.
 */
inline double ToMicroSecond(const Duration& duration )
{
    return static_cast<double>(AsInt64<nanos>(duration)) * 1e-6;
}
...
/**
 * @brief converts the input timestamp (nanos) to a double in million seconds.
 * The original precision will be preserved.
 * @param timestamp the input timestamp that needs to be converted.
 * @return a doule representing the same timestamp in million seconds.
 */
inline double ToMillionSecond(const Timestamp &timestamp)
{
    return static_cast<double>(AsInt64<nanos>(timestamp.time_since_epoch())) *
           1e-6;
}
...
/**
 * @brief converts the input timestamp (nanos) to a double in micro seconds.
 * The original precision will be preserved.
 * @param timestamp the input timestamp that needs to be converted.
 * @return a doule representing the same timestamp in micro seconds.
 */
inline double ToMicroSecond(const Timestamp &timestamp)
{
    return static_cast<double>(AsInt64<nanos>(timestamp.time_since_epoch())) *
           1e-3;
}
 ...
 */
template <typename PrecisionDuration>
inline Timestamp FromInt64(const int64_t timestamp_value)
{
    return Timestamp(
...
public:
    ...
    */
    ...
    /**
    * @brief gets the current time in second.
    * @return the current time in million second.
    */
    static double NowInMillionSeconds()
    {
        return ToMillionSecond(Clock::Now());
    }
    ...
    */
    ...
    {
        return ToMicroSecond(Clock::Now());
    }
    ...
    */
    static void SetNow(const Duration &duration)
    {
        ...
        {
            // << "Cannot set now when clock mode is not MOCK!";
        }
