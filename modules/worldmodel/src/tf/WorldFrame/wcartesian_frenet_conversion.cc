#include "modules/worldmodel/include/tf/WorldFrame/wcartesian_frenet_conversion.h"
...
void WcartesianFrenetConverter::cartesian_to_frenet(
...
void WcartesianFrenetConverter::frenet_to_cartesian(
        const double rs, const double rx, const double ry, const double rtheta,
...
double WcartesianFrenetConverter::CalculateTheta(const double rtheta,
                                                 const double rkappa,
                                                 const double l,
                                                 const double dl) {
    return math::NormalizeAngle(rtheta + std::atan2(dl, 1 - l * rkappa));
}

double WcartesianFrenetConverter::CalculateKappa(const double rkappa,
...
Vec2d WcartesianFrenetConverter::CalculateCartesianPoint(const double rtheta,
                                                         const Vec2d& rpoint,
                                                         const double l) {
...
double WcartesianFrenetConverter::CalculateLateralDerivative(
        const double rtheta, const double theta, const double l,
        const double rkappa) {
    return (1 - rkappa * l) * std::tan(theta - rtheta);
}

double WcartesianFrenetConverter::CalculateSecondOrderLateralDerivative(
    ...
    if (std::isinf(res)) {
//        //warn << "result is inf when calculate second order lateral "
