#include "modules/worldmodel/include/vehicle_state/vehicle_state_provider.h"

#include "modules/worldmodel/include/math/euler_angles_zxy.h"
#include "modules/worldmodel/include/math/quaternion.h"

#include <cmath>

#include "modules/common/include/util/string_util.h"
//#include "modules/localization/worldmodel/localization_gflags.h"
...
Status VehicleStateProvider::Update(
  ...
  if (!ConstructExceptLinearVelocity(localization)) {
      //due to the protobuf-lite,Debugstring is not allowed
    //std::string msg = atd::common::util::StrCat(,"localization:\n", localization.DebugString())
    return Status(atd::common::ErrorCode::LOCALIZATION_ERROR, "Fail to update because ConstructExceptLinearVelocity error.");
  }
  ...
  //vehicle_state_.set_localizationiving_mode(chassis.driving_mode());
...
const atd::common::VehicleState &VehicleStateProvider::vehicle_state() const {
