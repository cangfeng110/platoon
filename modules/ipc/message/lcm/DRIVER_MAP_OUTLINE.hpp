...
#ifndef __DRIVER_MAP_OUTLINE_hpp__
#define __DRIVER_MAP_OUTLINE_hpp__

#include <vector>
#include "DRIVER_MAP_POINT.hpp"


class DRIVER_MAP_OUTLINE
{
    public:
        ...
        std::vector< DRIVER_MAP_POINT > m_pointString;
...
int DRIVER_MAP_OUTLINE::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int DRIVER_MAP_OUTLINE::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int DRIVER_MAP_OUTLINE::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t DRIVER_MAP_OUTLINE::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* DRIVER_MAP_OUTLINE::getTypeName()
{
    return "DRIVER_MAP_OUTLINE";
}

int DRIVER_MAP_OUTLINE::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int DRIVER_MAP_OUTLINE::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int DRIVER_MAP_OUTLINE::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t DRIVER_MAP_OUTLINE::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == DRIVER_MAP_OUTLINE::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)DRIVER_MAP_OUTLINE::getHash };

    uint64_t hash = 0x51570acadf2e706aLL +
