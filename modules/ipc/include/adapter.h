#ifndef MODULES_IPC_ADAPTERS_ADAPTER_H_
#define MODULES_IPC_ADAPTERS_ADAPTER_H_
 ...
 * abstraction for Atd modules to interact with various I/O (e.g.
 * ROS). The adapter will also store history data, so that other
 * Atd modules can get access to both the current and the past data
...
public:
   ...
   */
    void OnReceive(const D& message,double cur_message_time) {
        if(!common::time::Clock::isSameTime(last_receive_time_,cur_message_time))
        {
            last_receive_time_ = cur_message_time;
            EnqueueData(message);
   ...
   */
    void FillHeader(const std::string& module_name, D* data,const double timestamp) {
    ...
    /**
   * @brief Gets message dhhelay.
   */
    double GetDelaySec() const override {
        if (last_receive_time_ == 0) {
            return -1;
        } else {
            return atd::common::time::Clock::NowInSeconds() - last_receive_time_;
        }
    }
...
private:
    ...
    bool FeedFile(const std::string& message_file, IdentifierType<T>) {
        D data;
        if (!atd::common::util::GetProtoFromFile(message_file, &data)) {
...
//        return atd::common::util::SetProtoToASCIIFile(
//                    message, atd::common::util::StrCat(dump_path_, "/", sequence_num, ".pb.txt"));
//    }
...
#endif  // MODULES_IPC_ADAPTERS_ADAPTER_H_
