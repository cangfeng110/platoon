...
void WorldModle::UpdateInfo()
{
    platoon_vehicles_info_ = UDPDataContainer::GetInstance()->platoon_vehicles_data_.getData(platoon_info_isupdate_);
    ego_vehicle_gps_info_ = HighFreDataContainer::GetInstance()->ego_vehicle_gps_data_.getData(ego_gps_isupdate_);
    worldmodle_other_vehicle_data_ = SendDataContanier::GetInstance()->worldmodle_other_vehicle_data_.getData(worldmodle_isupdate_);
}
/**
 * 
*/
void WorldModle::GetWorldmodleVehiles() 
{
    ...
    for(auto map_it : platoon_vehicles_info_) 
    {
        ...
        WorldModelObject worldmodel_vehicle_data = worldmodle_other_vehicle_data_[key].getData();
        ...
        worldmodle_other_vehicle_data_[key].setData(worldmodel_vehicle_data); 
    }
    worldmodel_other_vehicles_data_.vehicle_num = 0;
    worldmodel_other_vehicles_data_.vehicles.clear();
    for (auto map_it : worldmodle_other_vehicle_data_)
    {    
    ...
    if(ego_gps_isupdate_) 
    {
        for (int i = 0; i < trajectory.size(); i++) {
            platoon::common::TransfromGpsAbsoluteToEgoRelaCoord(trajectory[i].relative_x, trajectory[i].relative_y,
                                                                ego_vehicle_gps_info_.heading,
                                                                ego_vehicle_gps_info_.longitude, ego_vehicle_gps_info_.latitude,
                                                                ego_vehicle_gps_info_.height,
            ...
            platoon::common::TransfromGpsAbsoluteToEgoRelaAzimuth(trajectory[i].relative_heading, 
                                                                ego_vehicle_gps_info_.heading, trajectory[i].heading);
        }
    }
