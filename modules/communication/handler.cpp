...
int Handler::BroastEgoVehicleInfo() 
{
    ...
    if (ConfigData::GetInstance()->hmi_fms_valid_)
    {
        ...
        {
            const HmiFmsInfo& hmi_info = FmsData::GetInstance()->hmi_fms_info.getData();
            // this need to change, void data cast
            ego_vehicle_info.platoon_number = int8_t(hmi_info.platoon_number);
        }
        else
        {
            ego_vehicle_info.platoon_number = 0;
        }   
    }
    else
    {
        if (FmsData::GetInstance()->fms_pre_info_.isUpToDate()) 
        {
            const FMSPreFormationInfo& fms_pre_info = FmsData::GetInstance()->fms_pre_info_.getData();
             // this need to change, void data cast
            ego_vehicle_info.platoon_number = int8_t(fms_pre_info.platoonnumber());
        } 
        else 
        {
 ...
 * 2.0version
*/
int Handler::BroastEgoVehicleInfo(int) {
    //assign ego vehicle info
 ...
 * 2.0version
*/
int Handler::DecodeV2xVehicleInfo(int) {
