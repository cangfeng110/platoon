...
int PlatoonManagerInfo::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    if(this->vehicle_num > 0) {
        tlen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &this->vehicle_communication_status[0], this->vehicle_num);
        if(tlen < 0) return tlen; else pos += tlen;
    }
...
int PlatoonManagerInfo::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    if(this->vehicle_num) {
        this->vehicle_communication_status.resize(this->vehicle_num);
        tlen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &this->vehicle_communication_status[0], this->vehicle_num);
        if(tlen < 0) return tlen; else pos += tlen;
    }
