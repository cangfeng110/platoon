...
#include "platoon/protocol/WorldmodelVehicle.hpp"
...
int WorldmodelVehicles::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->nVehicleNum, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    for (int a0 = 0; a0 < this->nVehicleNum; a0++) {
...
int WorldmodelVehicles::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->nVehicleNum, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    this->vehicles.resize(this->nVehicleNum);
    for (int a0 = 0; a0 < this->nVehicleNum; a0++) {
...
uint64_t WorldmodelVehicles::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    uint64_t hash = 0xc1f556b0b1a9c027LL +
